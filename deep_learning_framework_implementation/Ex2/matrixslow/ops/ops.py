...
class Multiply(Operator):
    ...
    def get_jacobi(self, parent):
        ...
        # np.matrix.A1 = np.asarray(x).ravel()
...
class Convolve(Operator):
    ...
    def get_jacobi(self, parent):
        ...
        if parent is self.parents[0]:
            ...
            tmp = np.mat(jacobi)  # input_size * input_size
            ...
            tmp = np.mat(jacobi)  # input_size x filter_size
        else:
...
class Convolve(Operator):
    ...
    def compute(self):
        ...
        # 傅里叶反变换获得卷积结果
        self.value = np.real(np.fft.ifft2(result_fft))[:w, : h]
...
class Step(Operator):
    ...
    def get_jacobi(self, parent):
        ...
        return np.zeros(np.where(self.parents[0].value.A1 == 0.0, 1e8, 0.0))

class Weldingxx(Operator):
