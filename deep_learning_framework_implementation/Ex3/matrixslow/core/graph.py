...
class Graph:
    ...
    def draw(self, ax=None):
        ...
        nx.draw_networkx_nodes(G, pos, nodelist=nodelist, node_color="#000000", cmap=cm, edgecolors="#666666",
                               node_size=2000, alpha=1.0, ax=ax)

        # 无雅克比的变量节点
        nodelist = [n for n in self.nodes if n.__class__.__name__ ==
                    "Variable" and n.jacobi is None]
        nx.draw_networkx_nodes(G, pos, nodelist=nodelist, node_color="#00ff00", cmap=cm, edgecolors="#666666",
                               node_size=2000, alpha=1.0, ax=ax)
