...
class Saver(object):
    ...
    def create_node(graph, from_model_json, node_json):
        ...
        if node_type == 'Variable':
            ...
            dim = tuple(dim)
            #tmpnode = ClassMining.get_instance_by_subclass_name(Node, node_type)(*parents, dim=dim, name=node_name, **kargs)
            tmpnode = Variable(*parents, dim=dim, name=node_name, **kargs)
            return tmpnode
        else:
            tmpnode = ClassMining.get_instance_by_subclass_name(Node, node_type)(*parents, name=node_name, **kargs)
            return tmpnode

    def _restore_nodes(self, graph, from_model_json, from_weights_dict):
        for index in range(len(from_model_json)):
            node_json = from_model_json[index]  # 其中一个node的信息
            node_name = node_json['name']
